...
void load_toxic_words(const char* filename) {
    ...
    if (!file) {
        printf("错误：有toxicwords文件my_toxicwords.txt未找到，程序退出\n");
        exit(1);
    }
...
void generate_report(const char* filename) {
    ...
    for (int i = 0; i < top_n; i++) {
        const char* toxic_desc = word_list[i].is_toxic ?
            (word_list[i].severity == 1 ? "轻微toxic" :
                (word_list[i].severity == 2 ? "中等toxic" : "严重toxic")) : "正常";
...
int main() {
    ...
    while (1) {
        ...
        case 3:
            ...
            printf("唯一toxicwords数：%d\n", toxic_unique);
            if (toxic_unique == 0) {
                printf("未检测到toxicwords\n");
            }
            break;
        case 4:
